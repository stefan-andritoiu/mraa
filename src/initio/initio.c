...
static char** mraa_tokenize_string(const char* str, const char* delims, int *num_tokens)
{
...
static mraa_uart_ow_context parse_uart_ow(char **proto, size_t n)
{
  if (proto == NULL || n <= 1) {
...
static mraa_uart_context parse_uart(char **proto, size_t n)
{
  if (proto == NULL || n <= 1) {
  ...
  if (mraa_atoi_x(proto[idx], end, &bytesize, 0) != MRAA_SUCCESS) {
    syslog(LOG_ERR, "parse_uart: error setting up uart bytesize %d", bytesize);
    mraa_uart_stop(dev);
    return NULL;
  }
...
static mraa_spi_context parse_spi(char **proto, size_t n)
{
  if (proto == NULL || n <= 1) {
...
static mraa_pwm_context parse_pwm(char **proto, size_t n)
{
  if (proto == NULL || n <= 1) {
...
static mraa_iio_context parse_iio(char **proto, size_t n)
{
  if (proto == NULL || n <= 1) {
...
static mraa_i2c_context parse_i2c(char **proto, size_t n)
{
  if (proto == NULL || n <= 1) {
...
static mraa_aio_context parse_aio(char **proto, size_t n)
{
  if (proto == NULL || n <= 1) {
...
static mraa_gpio_context parse_gpio(char **proto, size_t n)
{
  if (proto == NULL || n <= 1) {
...
mraa_result_t mraa_io_close(mraa_io_descriptor* desc)
{
  ...
  if (desc->n_uart_ow) free(desc->uart_ows);
